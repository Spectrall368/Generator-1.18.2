...
public class ${name}TreeGrower extends <#if (data.megaTrees[0]?has_content) || (data.megaTrees[1]?has_content)>AbstractMegaTreeGrower<#else>AbstractTreeGrower</#if> {
	protected Holder<? extends ConfiguredFeature<?, ?>> getConfiguredFeature(Random randomSource, boolean hasFlower) {
	...
	protected Holder<? extends ConfiguredFeature<?, ?>> getConfiguredMegaFeature(Random randomSource) {
